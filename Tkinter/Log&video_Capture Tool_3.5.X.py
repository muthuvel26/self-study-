...
def getvalue(value):
    ...
    SV.set(serial.get())
    lock.release()
    t1 = threading.Thread(target=mn)
    t1.start()
    t2 = threading.Thread(target=bn)
    t2.start()
def mn():
...
def viewlog(i):
    if cnt!=0 and serial.current() != -1:
        ...
        messagebox.showinfo("ADB logs ", "Kindly Select Serial Number from the list box")
def start():
    if cnt!=0 and serial.current() != -1:
...
def exitlog():
    if cnt!=0 and serial.current() != -1:
        ...
        messagebox.showinfo("ADB logs ", "ADB log File:\n File->Open saved logs!!")
def stvideo():
    if cnt!=0 and serial.current() != -1:
...
def vstop():
    if cnt!=0 and serial.current() != -1:
        ...
        messagebox.showinfo("Screen record video ", "Screen video is saved in internal memory of the DUT!!")
def stop():
    subprocess.call('adb kill-server' ,shell=True)
    messagebox.showinfo("Thank you  ", "Credits:\n MUTHUVEL G and SOMC-OTC-Members")
    box.destroy()
def about():
    def astop():
        abox.destroy()
